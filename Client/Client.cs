    ...
    public partial class Client : Form
    {
            ...
            if (!exit)
            {
                connectButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg("Вы подключены к серверу"));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg("Вы отключены от сервера"));
                    }
                });
            }
        }
        ...
        private void ConnectButton_Click(object sender, EventArgs e)
        {
            ...
            else if (client == null || !client.IsAlive)
            {
                ...
                {
                    error = true;
                    Log(SystemMsg("Требуется указать IP-адрес"));
                }
                else
                {
                    try
                    ...
                    {
                        error = true;
                        Log(SystemMsg("IP-aдрес недействителен"));
                    }
                }
                int port = -1;
                if (number.Length < 1)
                ...
                {
                    error = true;
                    Log(SystemMsg("Требуется ввести имя пользователя"));
                }
                if (!error)
                {
